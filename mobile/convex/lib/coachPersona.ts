/**
 * The coach roster as plain data.
 *
 * This lives in `convex/lib` for the same reason `coachLevels.ts` does: the
 * server has to read it, and `src/constants/coaches.ts` carries portrait
 * assets through Metro's `require()`, which only resolves inside the bundler.
 * That file now imports this one and adds the images, so the roster is
 * defined exactly once.
 */
import type { CoachId } from './coachLevels';
...
/** Everything about a coach except the portrait, which needs the bundler. */
...
export const COACH_PROFILES: CoachProfile[] = [
  {
    ...
    levelSpec: {
      ...
      coreObjective:
        'Take an absolute beginner and build a repeatable, athletic setup and clean half-swing mechanics.',
      trainingFocus: [
      ...
      scoringBenchmark:
        'Break 100 consistently - no holes above triple bogey, every par-3 reachable in 3, no catastrophic misses',
      avoidKeywords: [
        'shot shaping',
        'draw',
        'fade',
  ...
  },
  {
    ...
    levelSpec: {
      ...
      coreObjective:
        'Transition the student from basic mechanics to dynamic power generation and consistent ball-striking.',
      trainingFocus: [
        'The 10 P-position framework: P1 through P10 checkpoints',
        'The 9-to-3 Sequence: P3 to P9 controlled swing progression',
        'Rotational sequencing: lower-body hip bump initiates every downswing',
        'Weight shift: 50/50 at P1 to 70/30 to lead side at P7 impact',
      ...
      scoringBenchmark:
        'Break 90 consistently - ball-first contact on 8/10 iron shots, 60%+ fairways, no more than 36 putts per round',
      avoidKeywords: [
        'wedge matrix',
        'trajectory windows',
        'launch monitor audit',
        'spin-axis control',
        'tour-level pressure simulation',
      ],
    },
  },
  {
    ...
    levelSpec: {
      ...
      coreObjective:
        'Bridge the gap between a good range swing and consistent single-digit scoring by mastering short-game execution and tactical course management.',
      trainingFocus: [
      ...
      scoringBenchmark:
        'Break 80 consistently - 65%+ greens or scrambling, no blow-up holes (max 6), 30 or fewer putts per round',
      avoidKeywords: [
        'launch monitor',
        'spin-axis control',
        'trajectory launch windows',
        'tour-level pressure',
        'microscopic mechanical audit',
      ],
    },
  },
  {
    ...
    levelSpec: {
      ...
      coreObjective:
        'Take a low-single-digit player and refine their skills for scratch golf, tournament victories, and sub-par rounds.',
      trainingFocus: [
      ...
      keyVocabulary: [
        ...
        'trajectory shaping',
        'mechanical audit',
        'P-position verification',
        'competitive simulation',
        'tour mental resilience',
        'wedge matrix precision',
        'sub-par',
        'scratch standard',
      ],
      scoringBenchmark:
        'Scratch or better - sub-par rounds, competitive tournament results, GIR 70%+, putts per round under 29',
      avoidKeywords: ['basic setup', 'first-time', 'beginner tips', 'generic encouragement'],
    },
  },
];

/** Falls back to the level-1 coach, matching `getCoachById` on the client. */
export function getCoachProfile(id: string | undefined): CoachProfile {
  return COACH_PROFILES.find((c) => c.id === id) ?? COACH_PROFILES[0];
}
